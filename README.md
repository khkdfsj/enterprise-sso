...
生产库已迁移 120 个 UserID、120 个当前任职和 3 名永久超级管理员；网页后台、年度换届、快捷注册与可回滚迁移已完成。当前只以“生日祝福后台”作为业务接入试点，enrollmentPhoto、StuReg 和其他原系统继续使用各自原认证。企业微信扫码复用原 CorpID、AgentID 与集中 access token；由于 118 不在企业微信可信 IP 列表内，UserID 查询通过 114 上受来源 IP和共享密钥双重保护的单文件桥接完成。
