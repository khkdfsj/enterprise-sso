...
function cookie(value, maxAge = 7200) {
  return `enterprise_admin=${value}; Path=/admin; HttpOnly; SameSite=Strict; Max-Age=${maxAge}${config.production ? '; Secure' : ''}`;
}
...
router.post('/login', adminLoginLimit, body, async (req, res) => {
  ...
  if (!result.ok) return res.status(401).send(page('登录失败', '<section><p>账号或密码错误，或账号当前不可用。</p><a href="/admin/login">返回</a></section>'));
...
router.get('/', requireAdmin, async (req, res) => {
  ...
  res.send(page('统一认证管理后台', `<h1>统一认证管理后台</h1><p>当前管理员：${esc(req.admin.person.display_name)}（${esc(req.admin.person.id)}）</p>
    ...
    <form method="post" action="/admin/logout"><input type="hidden" name="csrf" value="${csrf}"><button>退出后台</button></form>`));
});
...
router.post('/turnovers/start', requireAdmin, body, requireCsrf, async (req, res, next) => {
  try { await startTurnover(req.body.term_id, req.admin.person.id); res.redirect('/admin'); } catch (error) { next(error); }
});
...
router.post('/terms/:id/publish', requireAdmin, body, requireCsrf, async (req, res, next) => {
  try { await publishTerm(req.params.id, req.admin.person.id); res.redirect('/admin'); } catch (error) { next(error); }
});
