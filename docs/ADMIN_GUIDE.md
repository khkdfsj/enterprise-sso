...
1. 打开“接入服务管理 → 新增接入服务”。
2. 在第一步填写业务首页、精确登录回调、退出后返回地址和连通检测地址。
3. 第二步保存只显示一次的 `client_secret`，按文件名复制 `config.php`、受保护页面守卫、`callback.php`、`logout.php`、`health.php`、`test-login.php` 和 `test-logout.php`。
4. 第三步完成基础连通与凭据、真实登录认证、统一注销回跳三项验收。基础连通会持续监控十分钟；登录和注销测试成功后自动返回向导点亮状态。
5. 全部通过后进入服务详情，分别在“概览、登录与注销端点、访问权限、密钥与注册、连通监控”二级页维护，不把所有表单平铺在同一页。
