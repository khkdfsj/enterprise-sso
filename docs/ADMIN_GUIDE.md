...
日常管理统一使用：`http://210.47.163.114/enterprise-sso/admin`。后台按职责分为“概览、应用接入、人员与账号、届次换届、登录与审计、接入指南”六个模块。批量迁移、部署与应急操作继续使用 118 上的受控 CLI。生产环境文件位于：
...
1. 打开“应用接入”，点击“新建应用”。
2. 填写应用名称和精确回调地址；内网 HTTP 地址必须属于服务器配置的白名单主机。
3. 创建后立即复制 `client_id` 和只显示一次的 `client_secret`，将它们保存到业务程序的秘密配置中。
4. 进入应用详情，可继续添加或删除回调地址，选择“全部有效人员”或“按规则授权”，并按人员、部门或职位配置准入规则。
5. 如密钥泄露，在应用详情轮换密钥；旧密钥立即失效，新密钥仍只显示一次。
6. 只有需要业务系统代为发起开户时才开启快捷注册。业务系统只能取得 15 分钟单次注册链接，不能读取或设置用户密码。
