...
3. 第二步下载一次性 `ESSO-DFSJ.zip`，把完整的 `ESSO-DFSJ` 文件夹放到业务项目根目录，禁止更名。
   也可以点击“查看 Agent Skill，让 AI 协助快速接入”。所有运行部成员都可进入“管理 Agent 凭据”，为 Agent 绑定稳定身份并签发一次性可见的令牌；普通运行部成员只查看和撤销自己签发的凭据，平台管理员、老师和永久账户可管理全部凭据。令牌不得写入 Skill 或 GitHub；Agent API 生成的接入包与手动向导完全相同。
...
运行部成员可以查看接入服务并创建自己的服务；老师和永久账户可以查看全部服务。服务修改、检测和永久删除只允许服务创建者本人或平台管理员执行，部门职位不会自动获得修改他人服务的权限。删除需要进入独立确认页并输入完整服务名称；系统不会远程删除业务服务器上的 `ESSO-DFSJ` 文件夹。
...
打开“部门人员管理 → 届次换届”，按页面五步完成：

1. 确定当前届次、目标届次、新委员年级和生效时间；保存后草稿立即落库。
2. 从上届普通人员中勾选留任名单；永久用户自动沿用，无需选择。
3. 核对自动晋升结果：委员升副部长，副部长升部长；可逐人修改部门和职位。
4. 单人或批量新增委员；批量格式为“学号,姓名,部门名称”。
5. 核对最终表格并发布。未留任人员自动卸任和暂停，新委员初始密码为学号后六位，并写入“需要改密”标记。

流程支持断点续办：每一步均保存到数据库，离开页面后从“继续未完成换届”恢复；也可以返回上一步，不会清空已经录入的名单。未来日期发布不会提前停用现届人员，到生效时间才原子切换。详细规则见 [人员管理与换届](PERSONNEL_AND_TURNOVER.md)。

已经创建好的届次也可以在“人员与账号 → 批量新增委员”中追加名单。职位固定为委员，支持“学号,姓名,部门”格式；部门省略时使用页面选择的默认部门。整批校验成功后才会写入，已有账号保留密码，新账号使用学号后六位初始密码。
...
- 平台身份只有“普通平台用户”和“平台管理员”两类，与委员、副部长、部长、老师等部门职务完全独立。部长可以是普通平台用户，也可以被单独授予平台管理员身份。
- 平台管理员对应 `super_admin`，拥有全部后台权限并跳过部门职位限制；该身份不随届次自动失效，建议至少保留两名经过确认的负责人。
- 普通委员只读人员名单；副部长及以上可管理账号和办理换届；只有部长及以上可以移除普通人员。
- 运行部成员可查看接入服务；其他部门普通成员不显示接入服务区域。服务创建者只管理自己创建的服务。
- 老师、永久账户和平台管理员可查看全部后台模块，并可在人员的“平台身份”页面授予他人平台管理员或永久账户属性。为避免锁死，不能在该页面撤销当前登录账号自身的身份。
