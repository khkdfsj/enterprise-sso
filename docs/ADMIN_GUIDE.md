...
3. 第二步下载一次性 `ESSO-DFSJ.zip`，把完整的 `ESSO-DFSJ` 文件夹放到业务项目根目录，禁止更名。
   也可以点击“查看 Agent Skill，让 AI 协助快速接入”。部长以上、老师或后台管理员可进入“管理 Agent 凭据”，为 Agent 绑定稳定身份并签发一次性可见的令牌。令牌不得写入 Skill 或 GitHub；Agent API 生成的接入包与手动向导完全相同。
...
- 部长及以上、老师和人员管理员可维护普通人员；只有超级管理员可修改永久有效属性。
