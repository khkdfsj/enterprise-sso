...
114 原 Nginx 只增加一行独立 include；该文件只匹配 `/enterprise-sso` 和 `/enterprise-sso/`，不会接管其他路径，不开启 443、不做 HTTPS 跳转、不发送 HSTS。企业微信手机 OAuth 回调复用旧扫码系统目录中的独立 PHP 文件，仍从校园网 IP 的 80 端口访问。
